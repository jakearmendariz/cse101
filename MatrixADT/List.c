...
{
    Node *a = malloc(sizeof(Node) + sizeof(val));
    //a->value = malloc(sizeof(val));
    a->value = val;
    a->prev = NULL;
    a->next = NULL;
    return a;
...
// Returns cursor element of L. Pre: length()>0, indexItItItItIt()>=0
void *get(List L)
{
    if (L == NULL)
    {
    ...
    if (L->cursor_pos == -1)
    {
        //fprintf(stderr, "Error: cursor is undefinded (-1) in get() function\n");
        return NULL;
    }
    else if (L->length == 0)
    {
    ...
    if (L->cursor == NULL)
    {
        //printf(stderr, "Error: cursor is undefinded in get() function\n");
        return NULL;
    }
...
void *valueAt(List L, int pos)
{
    ...
    if (L->length < pos)
    {
        fprintf(stderr, "Cannot find value of indexIt in valueAt, list length is %d\n", L->length);
        return NULL;
    }
    int curr = 0;
    Node *a = L->front;
...
// Pre: length()>0, indexItItItItIt()>=0
// Other operations -----------------------------------------------------------
void delete (List L)
{
    //printf("deleteing %d\n", L->cursor->value);
    ...
    L->cursor_pos = -1;
    L->length--;
    Node *to_be_deleted = L->cursor;
    freeNode(to_be_deleted); //Sets cursor to NULL as well
}
...
void setCursor(List L, int at){
    ...
    //printf("Cursor suppose to be %d, but actually is %d\n", L->cursor_pos, at);
}